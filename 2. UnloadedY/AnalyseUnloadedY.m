...
%read in sim data
sdata1 = csvread('VerticalUnloadedSimulation.csv',1,0);
...
rdata1 = csvread('unloadedY1.csv',1,0);
...
rdata2 = csvread('unloadedY2.csv',1,0);
...
rdata3 = csvread('unloadedY3.csv',1,0);
...
rdata4 = csvread('unloadedY4.csv',1,0);
...
rdata5 = csvread('unloadedY5.csv',1,0);
...
legend('Trajectory Demand','Start Point','MyPAM R1','MyPAM R2','MyPAM R3','MyPAM R4','MyPAM R5','Simulated MyPAM','Location','northwest');
...
hold on
plot(timeSim1,aveY)
legend({'Trajectory Demand','MyPAM R1','MyPAM R2','MyPAM R3','MyPAM R4','MyPAM R5','Simulated MyPAM'},'Location','south','NumColumns',2);
xlabel('time (s)')
ylabel('Y-position (mm)')
xlim([0,timeSim1(end)]);
ylim([126.2-120,126.2+120]);
title('Y-position vs Time');
sgtitle({'Unloaded Simulated MyPAM and Real MyPAM','Responses to a Y-Direction Trajectory Trajectory Demand'})
...
title({'Unloaded Simulated MyPAM and Real MyPAM','Average Response to a Y-Direction Trajectory Demand'});
...
ylim([126.2-120,126.2+120]);
title('Y-position vs Time');
sgtitle({'Unloaded Simulated MyPAM and Real MyPAM Average','Response to a Y-Direction Trajectory Trajectory Demand'})